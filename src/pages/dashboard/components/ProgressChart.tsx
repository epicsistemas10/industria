    ...
    async function load() {
      ...
      try {
        ...
        for (const status of statusOrder) {
          ...
          sectorList.push({ name: status, count, percent, color: colorMap[status] });
        }
        ...
        for (const key of Object.keys(counts)) {
          if (!statusOrder.includes(key)) {
            ...
            const percent = total > 0 ? Math.round((count / total) * 100) : 0;
            sectorList.push({ name: key, count, percent, color: 'bg-gray-400' });
          }
        }
  ...
  return (
    <div className={`rounded-xl p-6 ${
      ...
      <div className="space-y-4">
        {loading && (
          <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Carregando dados...</div>
        )}
        {error && (
