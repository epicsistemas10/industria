-- Migração: adicionar coluna `categoria` na tabela `servicos` (se não existir)
-- e aplicar os valores de categoria com upsert para os códigos fornecidos.
...
-- This migration will: 
-- 1) add the `categoria` column if it's missing
-- 2) insert the list of services and update existing rows with the provided nome/categoria when codigo matches
